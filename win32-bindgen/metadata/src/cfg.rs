...
impl Cfg {
    ...
    pub fn union(&self, other: &Self) -> Self {
        ...
        self.types
            .keys()
            .for_each(|feature| union.add_feature(feature));
        other
            .types
            .keys()
            .for_each(|feature| union.add_feature(feature));
...
mod tests {
    ...
    fn relative() {
        ...
        let def = TypeReader::get()
            .expect_type_def(("Windows.Devices.Display.Core", "DisplayPresentationRate"));
        let namespaces = def.cfg().features("Windows");
        assert_eq!(namespaces.len(), 2);
        assert_eq!(namespaces[0], "Windows.Devices.Display.Core");
        assert_eq!(namespaces[1], "Windows.Foundation.Numerics");

        let def = TypeReader::get().expect_type_def((
        ...
        assert_eq!(namespaces[0], "Windows.Win32.Foundation");
        assert_eq!(namespaces[1], "Windows.Win32.Graphics.Direct3D");
        assert_eq!(namespaces[2], "Windows.Win32.Graphics.Direct3D11");
        assert_eq!(namespaces[3], "Windows.Win32.Graphics.Dxgi.Common");

        let def = TypeReader::get()
            .expect_type_def(("Windows.Win32.System.Diagnostics.Debug", "CONTEXT"));
        let namespaces = def.cfg().features("Windows");
        assert_eq!(namespaces.len(), 2);
    ...
    fn local() {
        ...
        let namespaces = def
            .cfg()
            .features("Windows.Win32.Security.Authorization.UI");
        assert_eq!(namespaces.len(), 1);
        assert_eq!(namespaces[0], "Windows.Win32.Foundation");
