...
fn codegen_dynamic_dispatcher<'ctx, 'a>(
    ...
    let cases: Vec<(IntValue, BasicBlock)> = lifted_functions
        ...
        .map(|&addr| {
            ...
            let fun_name = LlvmBuilder::get_name_for_sub(addr);
            let fun = if let Some(f) = module.get_function(&fun_name) {
                f
            } else {
                let f = module.add_function(&fun_name, types.bb_fn, Some(Linkage::External));
                f.set_call_conventions(FASTCC_CALLING_CONVENTION);
                f
            };
...
pub fn recompile<'ctx, 'a>(
    ...
    let split_module_builder = SplitModuleBuilder::new(context, &types, module_count);

    let mut queue = VecDeque::<(u32, BasicBlockSource)>::new();
    let mut lifted_functions = HashSet::new();
    queue.extend(
        basic_blocks
            .iter()
    ...
    while let Some((address, source)) = queue.pop_front() {
        if lifted_functions.contains(&address) {
            continue;
        }

        debug!("processing bb at 0x{:08x} (source = {:?})", address, source);

        let module_ctx = split_module_builder.get_module_for(address);

        let mut builder =
            LlvmBuilder::new(context, module_ctx, types.clone(), thunk_functions, address);

        lifted_functions.insert(address);
        ...
        let ret = loop {
            ...
            if let Some(addr) = flow.outer_jump_ref() {
                if !lifted_functions.contains(&addr) {
            ...
            if instr.op_code().code() == Call_rel32_32 {
                ...
                if !lifted_functions.contains(&target) {
                    queue.push_back((
                        target,
    ...
    let main_module_ctx = split_module_builder.get_main_module();
    let main_module = &main_module_ctx.module;
    ...
    codegen_dynamic_dispatcher(
        ...
        types.clone(),
        &lifted_functions,
        main_module_ctx.indirect_bb_call_impl,
        &main_module_ctx.rt_funs,
    );

    // codegen indirect_bb_call (exported as uwin_indirect_bb_call)
    codegen_dynamic_dispatcher_wrapper(
        context,
        main_module,
        types.clone(),
        main_module_ctx.indirect_bb_call_impl,
    ...
    codegen_thunk_finder(context, main_module, types, thunk_functions);

    split_module_builder.into_modules()
}

pub fn recompile_single_module<'ctx, 'a>(
    context: &'ctx Context,
    types: Arc<Types<'ctx>>,
    thunk_functions: &'a BTreeMap<u32, String>,
    image: &'a MemoryImage,
    ...
    recompile(context, types, thunk_functions, image, basic_blocks, 1)
        .pop()
        .unwrap()
}
