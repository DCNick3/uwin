...
public:
     ...
     */
    explicit kstream(std::unique_ptr<kin> io);

    explicit kstream(std::span<const std::uint8_t> data);
    ...
    static std::string bytes_to_str(std::string src, [[maybe_unused]] const std::string& src_enc);
