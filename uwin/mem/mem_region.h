...
namespace uwin {
    namespace mem {
        ...
        public:
            ...
            constexpr inline B begin() const { return _base; }
            constexpr inline B end() const { return _base + _size; }

            // checks whether the passed region is fully contained within this region
            // (region \subset this)
            constexpr inline bool does_contain(generic_mem_region<B, S> const& region) const {
                return begin() <= region.begin() && end() >= region.end();
            }

            // checks whether this region is fully contained within the passed region
            // (this \subset region)
            constexpr inline bool is_contained(generic_mem_region<B, S> const& region) const {
                return region.does_contain(*this);
            }

            constexpr inline bool intersects(generic_mem_region<B, S> const& region) const {
                return end() > region.begin() && region.end() > begin();
            }

            constexpr inline generic_mem_region<B, S> relative_to(generic_mem_region<B, S> const& region) const {
            ...
            inline bool operator==(generic_mem_region<B, S> const& o) const { return _base == o._base && _size == o.size(); }
...
template <> struct fmt::formatter<uwin::mem::hmem_region>: formatter<string_view> {
...
template <> struct fmt::formatter<uwin::mem::tmem_region>: formatter<string_view> {
    // parse is inherited from formatter<string_view>.
    template<typename FormatContext>
